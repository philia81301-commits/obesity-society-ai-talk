...
Firestore Database → **規則** 分頁 → 點進程式碼區域 → `Ctrl+A` 全選 → 貼上 → 按**發布**。

> ⚠️ 下面這份帶中文註解，複製過程若經過終端機或非 UTF-8 的編輯器會變亂碼。
> **貼進 Console 請改用純 ASCII 版：[`firestore.rules`](firestore.rules)**（功能完全相同，只有註解換成英文）。
> 另外「建立資料庫」精靈裡顯示的規則預覽是唯讀的，要等資料庫建好後在「規則」分頁才能編輯。
