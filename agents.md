...
- [x] 階段四：HTML 離線簡報製作 —— 50 頁骨架完成、視覺系統定案（板塊識別色＋SVG 圖標＋資料視覺化）、
      ...
      **P20／P25／P28／P31 四張工具截圖已嵌入**。**剩餘內容補件見 handoff.md**
...
- [x] 階段七：上台前定稿（2026-08-15 新增；**2026-09-08 結案**：P23 待補依使用者指示整塊移除、
      全簡報字級放大（base 42→46px，出處列固定 13px）、資料更新至 2026-08 月報＋維持期表單、
      PPTX v5 為現行繳交版，P6 於同日依原文 PMID 34335479 修正 n 值／圖例／引註）——**2026-08-16 完成四項**：
      PPTX 右下角頁碼（`build_pptx.py` 不再隱藏 `.slide-number`）、P9 政策段改寫、
      P22 改為整頁資訊呈現（滑桿移除）、P41 補入責任歸屬與醫療廣告界線。
      **2026-08-17（醫院電腦 X108521）再完成三項**：
      Firebase 已啟用（`live-config.js` 設定值已填，commit `99e2a7b`）、
      P9 依 ADA 官方 PDF 逐字掃描補入 §8.8a/§8.8b 的 A→E 落差、
      **新增 P27「門診實務」**（流程圖＋三張去識別化診間實拍，全篇由 50 頁變 51 頁）。
      **剩餘一項**：P23 敏感議題判定範圍（出處為 `減重門診AI管理.pptx` 第 9 張投影片備註，
      該檔在醫院電腦上仍未找到，需使用者自行定位或口述）
- [x] **階段八：肥胖醫學會會訊醫學專論（2026-08-18 新增並當日完成）**——
      任務為另投一篇會訊專論，體例照使用者前一篇〈體重管理的數位轉型〉（`.doc` 附件實測版面）。
      定稿 **12 頁、正文 4,198 字、4 圖 2 表、14 條引註、4 題自我評量附解析**。
      原始稿 `content/article-newsletter-2026.md`，排版程式 `article/build_docx.py`。
      DOCX 與 PDF 存 OneDrive 演講資料夾，**不進 git**（`.gitignore` 已擋 `*.docx`）。
...
- [x] ~~大綱 P29-31／P33-35「減重門診月報自動化＋遊戲」案例素材~~ 已定位並納入大綱（2026-08-14）
...
│   ├── osteoporosis-guideline-scan-20260815.md  ★骨鬆工具判定邏輯的指引溯源（P31 標註依據）
...
│   ├── fig1_flow.html                 圖一：減重門診 AI 生態系流程
│   ├── fig2_datalayer.html            圖二：資料蒐集自動化概念圖（原為實際截圖，改為概念圖）
│   └── figs/                          上述兩張的 PNG 輸出 ＋ 圖四組圖
└── slides/
    ├── index.html                     ★簡報本體（51頁，離線可播）
